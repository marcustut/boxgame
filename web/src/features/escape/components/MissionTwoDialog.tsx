...
const missionTwoSchema = Yup.object({
  answer: Yup.string()
    .required('Answer cannot be empty')
    .matches(/^[0-9]+$/, 'Must be only digits')
})

type MissionTwoDialogProps = {
  open: boolean
  onClose: () => void
}

export const MissionTwoDialog: React.FC<MissionTwoDialogProps> = ({ open, onClose }) => {
  const { user } = useAuth()
        ...
        >
          <div className='flex flex-col justify-center items-center fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 <sm:w-4/5 w-96 px-6 py-4 bg-dark-400/90 rounded-lg shadow-2xl'>
            <Formik
              ...
              validationSchema={missionTwoSchema}
              ...
              onSubmit={async ({ answer }) => {
                ...
                const { data, errors } = await upsertEscape({ teamId: user.user.team.id, missionTwo: true })
                if (errors || !data) {
